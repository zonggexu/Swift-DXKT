...
/// TableViewSection的默认高度 0.01
let kTableViewNoneSectionHeight = 0.01

func CommonWidth(_ width:CGFloat) -> CGFloat {
    return width*1.0/375 * kScreenWidth
}
func CommonHeight(_ height:CGFloat) -> CGFloat {
    return height*1.0/667 * (kScreenHeight - 2 * kTopSafeHeight)
}
...
func STATUSBAR_HEIGHT() ->CGFloat {
    if #available(iOS 11.0, *) {
        return getWindow()?.safeAreaInsets.top ?? 20
    } else {
...
// MARK: - 机型
/// 当前设备类型
let iOS_DEVICE_MODELNAME = UIDevice.current.model
/// 当前设备型号
let iOS_DEVICE_DEVICENAME = Device.current

/// 判断当前设备是否是iPhone
let Jh_Is_iPhone = UIDevice.current.userInterfaceIdiom == .phone
/// 判断当前设备是否是iPad
