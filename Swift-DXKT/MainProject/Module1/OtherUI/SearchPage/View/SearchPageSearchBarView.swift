...
class SearchPageSearchBarView: UIView {
    ...
    func setupViews() {
        textField.snp.makeConstraints { (make) in
            make.bottom.equalTo(-(kContentNavBarHeight - 33.0) / 2.0);
    ...
    lazy var textField: UITextField = {
        ...
        textField.tintColor = BaseThemeColor
        ...
        searchIcon.snp.makeConstraints({ (make) in
            make.center.equalTo(leftView.snp.center).offset(0)
        })
    ...
    lazy var cancleBtn: UIButton = {
        ...
        cancleBtn.setTitleColor(BaseThemeColor, for: .normal)
        cancleBtn.titleLabel?.font = JhFont(14)
...
extension SearchPageSearchBarView: UITextFieldDelegate {
    ...
    func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        let leftView = UIView(frame: CGRect(x: 0, y: 0, width: 30, height: 33))
        let searchIcon = UIImageView(image: UIImage(named: "common_search_bar_search_icon"))
        searchIcon.frame = CGRect(x: 0, y: 0, width: 15, height: 17)
